...
SECRET_NAME = "awsedb-mdids-ihub-data-harmony-clariness-login-secrets"
DB_SECRET_NAME = "awsedb-mdids-ihub-data-harmony-clariness-secrets"
ABC_DB_SECRET_NAME = "awsedb-auroradb-secrets"
GLUE_JOB_NAME = "edb_mdids_clariness_data_ingestion"
S3_BUCKET = "lly-edp-raw-us-east-2-dev"
OUTPUT_S3_KEY = "mdids-infohub/clariness_test/results/"
FLAG_S3_KEY = "mdids-infohub/clariness_test/input/clariness_test_flag.json"
...
def get_secret(secret_name: str):
    ...
    return {
        "host": secret_lower.get("host") or secret_lower.get("db_host"),
        "port": secret_lower.get("port", 5432), 
        "dbname": secret_lower.get("dbname") or secret_lower.get("db_name"),
        "user": secret_lower.get("username") or secret_lower.get("user"), 
...
def lambda_handler(event, lambda_context):
    ...
    # NEGATIVE FLOW
        ...
        try:
            # Extract DB snapshot before invalidation
            try:
            ...
            if glue_end_time is not None:
                try:
                    ...
                    # Extract DB snapshot after glue run and compare
                    try:
                        db_config_tmp = get_db_config(DB_SECRET_NAME)
            ...
            # Upload output JSON directly to S3
            try:
